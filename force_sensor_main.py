...
from ForceSensor.filters import LowPassFilter
from ForceSensor.visualization import ForceVisualizer
from ForceSensor.data_writer import DataWriter
import keyboard
...
def data_acquisition(sensor, data_queue):
...
def data_processing(data_queue, filter):
...
def key_listener(sensor, zero_calibration_event,exit_event, recording_event):
    ...
    try:
        ...
        pass
# 20250325 just zero and record and visualization
if __name__ == "__main__":
    sensor= ATIMini85()
    sensor.start(sampling_rate=SAMPLING_RATE)
    print("zero offset calibration")
    sensor.calibrate_zero() # expect the first time stable zero calibration, no zero offset again
    ...
    # start the thread
    threads = [
        threading.Thread(target=data_acquisition, args=(sensor, data_queue)),
        threading.Thread(target=data_processing, args=(data_queue, filter))
    ]
    ...
    # visulation maybe clog the mian thread
...
if __name__ == "__main__":
    ...
    listener.join()
    """
# 20250324 read the data, visualization and use keyboard to store to csv file, no filter, !! because of the data speed is 10ms, so keyboard hard to read,change SAMPLE_CHUNK to use keyboard
"""
